...
independent_variables_add[0] += ["LAST_LETTER", 'TYPE', 'MAXPOSITION', "DIGITS", 'POSITION']
...
def getCosine(vectorA, vectorB):
    return np.dot(vectorA, vectorB) / (norm(vectorA, axis=0) * norm(vectorB))
...
def createModel(words, pklFile=""):
    ...
    joblib.dump(model, "output/gensimTrainingModel.pkl")
...
def annotate_word(normalized_length, code_context, last_letter, word_type, max_position, digits, position):
    input_model = 'model_DecisionTreeClassifier.pkl'
    ...
    df_features = pd.DataFrame(data,
                               columns=['NORMALIZED_POSITION', "LAST_LETTER", 'TYPE', 'MAXPOSITION', "DIGITS",
                                        'POSITION'])
    # df_features = pd.DataFrame(data,
...
def read_from_database():
    ...
    print("IDENTIFIER,GRAMMAR_PATTERN,WORD,SWUM,STANFORD,CORRECT,PREDICTION,MATCH,SYSTEM,CONTEXT,IDENT",
          file=open("output/model_DecisionTreeClassifier_predictions.csv", "a"))
    ...
    for i, row in df_input.iterrows():
        ...
        result = annotate_word(normalized_length, code_context, last_letter, word_type, max_position, digits, position)
        print(
            "{identifier},{pattern},{word},{swum},{stanford},{correct},{prediction},{agreement},{system_name},{context},{ident}"
            .format(identifier=actual_identifier, word=actual_word, pattern=actual_pattern,
                    ...
                    word_type=word_type, max_position=max_position, digits=digits, position=position),
            file=open("output/model_DecisionTreeClassifier_predictions.csv", "a"))
